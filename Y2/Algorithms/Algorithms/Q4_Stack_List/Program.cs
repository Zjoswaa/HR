...
for (int i = 1; i <= 10; ++i) {
    if (i % 3 == 0)
        Console.WriteLine($"Peek() => {(stk.Peek() == null ? "<NULL>" : stk.Peek())}, after Peek -> Count: {stk.Count}");
    stk.Push(i + "");
    Console.WriteLine($"Push({i}); after Push -> Count: {stk.Count} ");
    if (i % 5 == 0) {
        var val = stk.Pop();
        Console.WriteLine($"Pop() => {val ?? "<NULL>"}, after Pop -> Count: {stk.Count}");
    }
}
...
var res = stk.Pop();
Console.WriteLine($"Pop() => {res ?? "<NULL>"}, after Pop -> Count: {stk.Count}");
res = stk.Peek();
Console.WriteLine($"Peek() => {res ?? "<NULL>"}, after Peek -> Count: {stk.Count}");
